...
class DataIngestion:
    ...
    def initiate_data_ingestion(self):
        logging.info("entered the data ingestion meythod or component")
        try:
            df=pd.read_csv('notebook\data\stud.csv')
            logging.info("read the dataset as dataframe")
            os.makedirs(os.path.dirname(self.ingestion_congif.train_data_path))
            ...
            test_set.to_csv(self.ingestion_congif.train_data_path,index=False,header=True)
            logging.info("inmgestion ofthe data is completed")
            return(
